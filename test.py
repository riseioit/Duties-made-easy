import os
import pandas as pd
os.chdir("/home/vaibhavgole/workspace/Building_Desktop_App/Electron/dutisMadeEasy")
import sortfuction as sf
nfile = "DataFile2.xlsx"
file=pd.read_excel(nfile,sheet_name="Sheet1",na_values=" ")
#print(file)


####################filepath=pfile###    Accissing the Required Data    ########################
...
teacherfile=pd.read_excel(nfile,sheet_name="old Jr. Sup. order Nov. 2019", na_values=" ")
#deleterow(3)
to_delete=[0,1,2]
teacherfile.drop(teacherfile.index[to_delete], inplace=True)

colname=teacherfile.loc[3,:]
#print("line 25: colname\n", colname)
...
fav=[]
favdt=pd.read_excel(nfile,sheet_name="Sheet3", na_values=" ")
#print(favdt.columns)
...
#        print("/nline 163: count=",count,"index = ",index)
        ...
        if count < dayduty[index][1] and count>=0:
            if dutyalloted<dutyreq:
                ...
                if name in teacherlist:
                    ita=teacherlist.index(name)
                    tarikh=day[index][0][0:len(day[index][0])-1]
                    if tarikh != fav[ita][2]:
...
for i in range(len(forteacher)):
    newlist.clear()
    sorteddates[i][1] = sf.sortkar(forteacher[i][1])
...
writer.save()
print("________________________Code execution Successful_________________________")
